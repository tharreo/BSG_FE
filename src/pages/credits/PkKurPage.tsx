...
export function PkKurPage() {
  ...
  const [selectedData, setSelectedData] = useState<IResListPkKur | undefined>();
  const [dataList, setDataList] = useState<IResListPkKur[]>([]);
  ...
  function fetchData(param?: string) {
    dispatch(creditActions.getListPkKur(param)).then();
  }

  useEffect(() => {
    setDataList(credit?.listPkKur?.data || []);
    setPaginatedData(credit?.listPkKur?.paginated_data || defaultPaginatedResponse);
    setLoadingPage(credit?.listPkKur?.loading || false);
  }, [credit.listPkKur]);

  useEffect(() => {
    console.log(credit?.listPkKur?.paginated_data);
  }, [credit?.listPkKur?.paginated_data]);
  ...
  const tableColumnData: ITableColumnData[] = [
    {
      ...
      headerTitle: '#',
      align: 'left',
      layouts: (e: IResListPkKur) => <div>{e.number}</div>,
    },
    {
      key: 'request-number',
    ...
    },
    {
      ...
      align: 'left',
      layouts: (e: IResListPkKur) => <div>{e.plafond ? numberFormat.toRupiah(e.plafond) : '-'}</div>,
    },
    {
      key: 'actions',
      headerTitle: '',
  ...
  return (
    <>
      ...
      <div className={'flex justify-between'}>
        <PageTitle title={t('commercial-credit-pk-kur')} />
        <Link to={ROUTES.NEW_PK_KUR()}>
      ...
      <CommercialCreditTabButton activeIndex={'PK KUR'} />
